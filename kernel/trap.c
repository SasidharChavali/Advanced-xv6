...
usertrap(void)
{
  ...
  if(which_dev == 2) 
  {
    ...
    { 
      ...
      { 
          p->alarm_ticks = p->alarm_intervals;
          // jump to execute alarm_handler
          memmove(p->user_saved_tf,p->trapframe,sizeof(struct trapframe)); // backup trapframe
          p->trapframe->epc = (uint64)p->alarm_handler_addr;
          p->alarm_on = 1;
