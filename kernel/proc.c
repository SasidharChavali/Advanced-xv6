...
found:
  ...
  if((p->user_saved_tf = (struct trapframe *)kalloc()) == 0){
    release(&p->lock);
    return 0;
  }
...
procdump(void)
{
  ...
  char *state;
    ...
    printf("\nPID\tState\trtime\twtime\tnrun\t creationtime");
  #endif
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      printf("%d\t%s\t%d\t%d\t%d\t%d", p->pid, state, p->rtime, end_time - p->ctime - p->rtime , p->no_of_times_scheduled,p->ctime);
      printf("\n");
    #endif
    #ifdef LBS
     printf("%d\t%d\t%s\t\n", p->pid, p->tickets, state);
    #endif
...
int alarm(int n, void(*fn)()) {
  struct proc *p = myproc();
  p->alarm_intervals = n;
  p->alarm_handler_addr = (uint64)fn;
  p->alarm_ticks = n;
  return 0;
}
...
int sigreturn(){
  struct proc *p = myproc();
  memmove(p->trapframe,p->user_saved_tf,sizeof(struct trapframe));
  p->alarm_on = 0;
